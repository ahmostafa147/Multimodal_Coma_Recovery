"""
06_patient_analysis_v2.py — full CEBRA analysis for one patient (v2 visuals).

v2 differences vs cebra_pipeline/06: companion boundaries-only figure,
Plasma colormap on trajectory (hours-from-start), hour-labeled start/end
markers.
...
# Reuse cebra_pipeline/_constants for single source of truth.
# __file__ is undefined in Jupyter cells, so fall back to cwd and search.
try:
    _here = os.path.dirname(os.path.abspath(__file__))
except NameError:
    _here = os.getcwd()
for _p in (os.path.join(_here, 'cebra_pipeline'), _here,
           os.path.join(os.path.dirname(_here), 'cebra_pipeline')):
    if os.path.isfile(os.path.join(_p, '_constants.py')):
...
OUT_ROOT = f'{OUT_DIR}/patient_analysis_v2_{RUN_TAG}'
...
# midpoint time in hours
t_h = (np.cumsum(p_dur) - p_dur / 2) / 3600.0

# centroids from train
centroids = np.zeros((N_CLS, train['emb'].shape[1]))
...
sojourns = {c: [] for c in range(N_CLS)}
if N > 0:
    s, run = p_y[0], 1
    for i in range(1, N):
        if p_y[i] == s:
...
# ── helpers ─────────────────────────────────────────────────────────
def slerp(p0, p1, t):
    """Spherical linear interpolation between two 3D points, preserving radius."""
    ...
    if np.sin(omega) < 1e-6:
        ...
        u = (np.sin((1 - t) * omega) / np.sin(omega)) * u0 \
          + (np.sin(t * omega)       / np.sin(omega)) * u1
    return u * ((1 - t) * r0 + t * r1)
...
rep_p, rep_t, rep_l, rep_state, rep_h = [], [], [], [], []
smooth_p = smooth_t = smooth_h = np.empty(0)
if N >= 2:
    for ws in range(0, N, WINDOW):
        ...
        rep_p.append(wc[last])
        rep_t.append((ws + last) / max(N - 1, 1))
        rep_h.append(t_h[ws + last])
        rep_l.append(CLASS_NAMES[win]); rep_state.append(win)
    rep_p = np.array(rep_p); rep_t = np.array(rep_t); rep_h = np.array(rep_h)

    sp, st, sh = [], [], []
    for i in range(len(rep_p) - 1):
        for s in range(SLERP_N):
            tt = s / SLERP_N
            sp.append(slerp(rep_p[i], rep_p[i+1], tt))
            st.append(rep_t[i] + tt * (rep_t[i+1] - rep_t[i]))
            sh.append(rep_h[i] + tt * (rep_h[i+1] - rep_h[i]))
    sp.append(rep_p[-1]); st.append(rep_t[-1]); sh.append(rep_h[-1])
    smooth_p = np.array(sp); smooth_t = np.array(st); smooth_h = np.array(sh)

# ── BUILD UNIFIED PLOT ─────────────────────────────────────────────
fig = go.Figure()

# A. train embeddings colored by CPC outcome
fig.add_trace(go.Scatter3d(
    x=train['emb'][g_idx,0], y=train['emb'][g_idx,1], z=train['emb'][g_idx,2],
    mode='markers',
    marker=dict(size=1.6, color=OUTCOME_GOOD_RGB, opacity=0.18),
    name='Good outcome (CPC<=2)'))
fig.add_trace(go.Scatter3d(
    x=train['emb'][b_idx,0], y=train['emb'][b_idx,1], z=train['emb'][b_idx,2],
    mode='markers',
    marker=dict(size=1.6, color=OUTCOME_BAD_RGB, opacity=0.18),
    name='Poor outcome (CPC>=3)'))

# B. curved cluster boundaries + floating state names (display order)
for c in DISPLAY_ORDER:
    mask_c = train['y'] == c
    if mask_c.sum() < 20:
        continue
...
for c in DISPLAY_ORDER:
    ...
    fig.add_trace(go.Scatter3d(
        ...
        mode='markers',
        marker=dict(size=5, color=CLASS_COLORS[c], symbol='circle',
...
# D. patient trajectory — Plasma over hours, halo for pop, hour-labeled markers
if N >= 2:
    fig.add_trace(go.Scatter3d(
    ...
    fig.add_trace(go.Scatter3d(
        x=smooth_p[:,0], y=smooth_p[:,1], z=smooth_p[:,2],
        mode='lines',
        line=dict(color=smooth_h, colorscale='Plasma', width=10,
                  cmin=float(smooth_h.min()), cmax=float(smooth_h.max()),
                  showscale=True,
                  colorbar=dict(title='Hours from start',
                                x=1.02, len=0.55, thickness=14,
                                tickfont=dict(color='white'),
                                title_font=dict(color='white'))),
        name=f'{PID} trajectory'))
    fig.add_trace(go.Scatter3d(
        x=[smooth_p[0,0], smooth_p[-1,0]],
        y=[smooth_p[0,1], smooth_p[-1,1]],
        z=[smooth_p[0,2], smooth_p[-1,2]],
        mode='markers+text',
        marker=dict(size=10, symbol='diamond',
                    color=['#440154', '#fde725'],
                    line=dict(color='white', width=1.5)),
        text=['0 h', f'{smooth_h[-1]:.1f} h'],
        textposition='top center',
        textfont=dict(size=12, color='white'),
        showlegend=False))

# extent for symmetric range
...
extent_pts.append(np.array(label_positions))
if N >= 2:
...
# ── boundaries-only companion figure (no trajectory) ──────────────
fig2 = go.Figure()
...
fig2.update_layout(
    title=f'CEBRA state regions — {PID} [{RUN_TAG}]',
    scene=dict(
        bgcolor='rgb(20,25,40)', aspectmode='cube',
        xaxis=dict(title='Dim 1', range=rng_sym, **axis_style),
        yaxis=dict(title='Dim 2', range=rng_sym, **axis_style),
...
fig2.write_html(os.path.join(OUT, '02_unified_boundaries.html'))
print('Saved 02_unified_boundaries.html')

# ── 4. state timeline (display order on y-axis) ───────────────────
...
ax.set_yticks(range(N_CLS)); ax.set_yticklabels(DISPLAY_NAMES)
...
# ── 5. state distribution (display order) ─────────────────────────
...
# ── 6. centroid similarity heatmap (display order) ────────────────
...
im = ax_h.imshow(sims_disp.T, aspect='auto', cmap='RdBu_r', vmin=-1, vmax=1,
                 extent=[t_h[0], t_h[-1], N_CLS - 0.5, -0.5])
ax_h.set_yticks(range(N_CLS)); ax_h.set_yticklabels(DISPLAY_NAMES)
...
ax_b.legend(handles=patches, loc='upper center',
            bbox_to_anchor=(0.5, -1.6), ncol=N_CLS,
...
ax.set_xticks(np.arange(N_CLS)); ax.set_yticks(np.arange(N_CLS))
